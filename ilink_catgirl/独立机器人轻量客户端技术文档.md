...
用户主动询问当前任务时，流程与到点发送不同。模型先调用 `list_reminders`，`ReminderTools._list()` 从 SQLite 实时读取任务，并返回编号、内容、任务类型、本地展示时间、重复方式和状态。工具结果中的 `presentation_hint` 以及根目录 `聊天助手.txt` 要求模型保持数据准确，再用爱丽丝的RPG任务日志口吻自然组织，而不是逐项朗读“字段：值”。因此任务数量来自数据库，任务名称式表达可以由模型根据内容自然概括。
