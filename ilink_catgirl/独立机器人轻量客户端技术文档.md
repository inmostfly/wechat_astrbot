...
| `weixin_ilink.py` | 实现登录二维码、消息长轮询、微信 CDN 图片下载与 AES 解密、文本发送和会话持久化 |
...
→ 调用 deepseek-v4-flash-vision-exp
→ 丢弃图片字节和 data URL
```
...
视觉请求使用一份临时消息列表，图片数据不会追加到 `ReplyEngine.memories`，聊天日志也只记录“图片×数量”。请求成功后，程序只把用户的文字附言（无附言时写入文字占位）和视觉模型的文字回答加入滑动上下文。因此后续模型知道刚才的识别结论，但不能重新读取原图。程序也不会把收到的图片保存到磁盘。
...
运行依赖集中在 `requirements.txt`。其中 `mcp[cli]` 使用完整 MCP 可选依赖，确保 PyInstaller 能完整收集 MCP 包；`cryptography` 用于微信媒体 AES 解密；`random`、`sqlite3`、`threading`、`zoneinfo` 等均为 Python 标准库，不需要写入 requirements。打包工具单独放在 `requirements-build.txt`，避免把 PyInstaller 混成普通运行依赖。

自动化测试覆盖消息解析、AES 解密、图片格式识别、视觉模型路由和图片不进入长期上下文。由于测试不会操作线上微信，部署后的第一次实测仍应观察：
...
6. 天气问题是否在日志中出现 `TOOL:get_weather`；
7. 搜索问题是否出现 `TOOL:search_web`，发送网址是否出现 `TOOL:read_webpage`；
8. 输入 `quit` 后是否正常下线。
...
当前版本已经支持接收和识别图片，但回复仍然是文字。主动发送图片、语音、文件和视频仍需实现 `getuploadurl`、AES 加密、CDN 上传和媒体消息引用；语音、文件、视频的入站解析也尚未接入。其他可选改进包括：
