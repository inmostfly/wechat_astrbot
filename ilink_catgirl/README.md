...
- MCP 工具调用 JSON 只存在于当次请求，长期滑动上下文仅保留稳定的用户消息和最终回答
...
EXE 成品会把 `聊天助手.txt`、`主动问候语.txt` 和 `定时提醒开场白.txt` 集中放在同目录的 `用户自定义` 文件夹。程序优先读取这些外置文件，保存修改并重启后生效；文件缺失时会退回打包时内置的默认版本。源码运行仍可直接编辑项目原有的同名文件。
