"""Synchronous bridge to the local stdio web MCP server."""
...
SERVER_PATH = Path(__file__).with_name("web_mcp_server.py")
...
def list_web_tools() -> list[dict[str, Any]]:
        ...
        async with stdio_client(server_parameters(), errlog=errlog) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                if result.isError:
                    ...
                    raise RuntimeError(
                        "\n".join(texts) or f"联网 MCP 工具 {name} 调用失败"
                    )
...
def call_web_tool(name: str, arguments: dict[str, Any]) -> str:
