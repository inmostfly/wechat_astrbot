...
2. 通过微信 UI Automation 读取消息，并在发送瞬间获取输入焦点，不持续抢占前台。
...
项目早期曾使用前台守护线程和微信右上角的“置顶”按钮，防止用户操作其他程序时
微信失去焦点。现在程序运行在专用机器上，这些持续置前功能已全部移除。

当前 `WeChat()` 初始化时只执行一次 `BringToFront()`，确保微信窗口已经恢复并可被
UI Automation 访问。程序运行期间不会循环抢占前台，也不会打开或关闭微信的窗口
置顶状态。发送消息时 `SendMsg()` 会点击输入框，再执行 `Alt+S`；这是完成单次发送
所需的临时焦点，不属于持续前台守护。

`Close()` 不再访问微信 UI，只结束适配层生命周期，因此程序退出时不会因为尝试
“取消置顶”而产生清理阶段的二次 UIA 异常。
