...
天气查询功能、联网搜索功能和常见文档解析功能，同时本地建立数据库，支持设置每日任务，到时间后自动在微信发送消息提醒。

微信收到图片时会在内存中下载并解密，按需调用 `deepseek-v4-flash-vision-exp` 识别；原图和 base64 不写入磁盘，也不保存进长期对话上下文。

微信收到 PDF、DOCX、XLSX、PPTX 或常见纯文本文件时，会在内存中下载和解密，再交给本地文档 MCP 提取文字。提取内容进入当前滑动上下文，可以继续追问；原始文件不会保存到项目目录。
