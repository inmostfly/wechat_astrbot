# Catgirl 微信 UI Automation 归档版技术文档

更新日期：2026-08-15
适用环境：Windows、微信 4.x、Python 3.10+  
当前状态：此 UI Automation 方案已经归档；历史上完成过微信收发、窗口置前、聊天日志和天气 MCP 的实际测试。当前主线位于 `../ilink_catgirl`。
...
UIA 专属文件已经迁移到本目录；天气 MCP、日志模块和系统提示词继续由父目录共享。

| 文件 | 作用 |
| --- | --- |
| `my_catgirl.py` | 主程序；监听微信、维护上下文、调用模型、执行工具并发送回复 |
| `wechat_uia.py` | 自研微信 4.x UI Automation 适配层 |
| `../chat_logger.py` | 父目录共享日志模块，按启动时间创建聊天日志 |
| `../weather_mcp_server.py` | 父目录共享的和风天气 MCP 服务端 |
| `../weather_mcp_client.py` | 父目录共享的 MCP 客户端桥接 |
| `../聊天助手.txt` | 父目录共享的大模型系统提示词 |
...
默认在项目父目录的 `.env` 中配置。也可以在 `UIA/.env` 中写 UIA 版专用覆盖值；不要把实际密钥写进文档或提交到 Git。
...
```powershell
python -m pip install -r UIA/requirements.txt
```
...
也可以直接双击 `UIA/启动微信助手.bat`。它优先使用 `UIA/.venv`，否则使用
PATH 中的 Python；也可通过 `CATGIRL_PYTHON` 指定已有虚拟环境，不需要打开 VS Code。
...
双击 `UIA/打包EXE.bat`。脚本会安装 UIA 运行与打包依赖，然后根据
`catgirl.spec` 生成目录版程序：

```text
UIA\dist\Catgirl微信助手\Catgirl微信助手.exe
```
